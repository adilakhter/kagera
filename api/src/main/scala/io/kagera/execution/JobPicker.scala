 ...
 */
class JobPicker[P[_], T[_, _]](tokenGame: TokenGame[P[_], T[_, _], Marking[P]]) {
      ...
      instance.isBlockedReason(transition) match {
        ...
        case None ⇒
          tokenGame.enabledParameters(instance.process)(instance.availableMarking).get(transition) match {
            ...
            case Some(params) ⇒
              val (updatedState, job) = createJob(transition.asInstanceOf[T[Any, E]], params.head, input)(instance)
   ...
   */
  private def createJob[E, S](transition: T[Any, E], consume: Marking[P], input: Any): Instance[P, T, S] ⇒ (Instance[P, T, S], Job[P, T, S, E]) = instance ⇒ {
  ...
  def isFireable[S](instance: Instance[P, T, S], t: T[_, _]) = {
